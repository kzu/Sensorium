...
// Note: requires additional types generated from: <xsl:value-of select="."/></xsl:template>
  ...
  <xsl:template match="FieldDescriptorProto[label='LABEL_OPTIONAL' or not(label)]">
    ...
    [ComponentModel.DefaultValue(<xsl:value-of select="$defaultValue"/>)]</xsl:if><!--
    --><xsl:if test="$optionXml">
  ...
  <xsl:template name="WriteGetSet">
    ...
    <xsl:if test="$optionFullFramework">[ComponentModel.Browsable(false)]</xsl:if>
  ...
  <xsl:template match="FieldDescriptorProto[label='LABEL_REPEATED']">
    ...
    <xsl:variable name="format"><xsl:apply-templates select="." mode="format"/></xsl:variable>
    <xsl:variable name="field"><xsl:apply-templates select="." mode="field"/></xsl:variable>
    private <xsl:if test="not($optionXml)">readonly</xsl:if> Collections.Generic.List&lt;<xsl:value-of select="$type" />&gt; <xsl:value-of select="$field"/> = new Collections.Generic.List&lt;<xsl:value-of select="$type"/>&gt;();
    ...
    public Collections.Generic.List&lt;<xsl:value-of select="$type" />&gt; <xsl:call-template name="pascal"/>
    {
  ...
  <xsl:if test="$optionAsynchronous and $optionDataContract">
    public partial class <xsl:value-of select="name"/>CompletedEventArgs : ComponentModel.AsyncCompletedEventArgs
    {
